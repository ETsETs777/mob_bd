#!/usr/bin/env python3
"""Generate lib/l10n/app_{locale}.arb from app_en.arb (batch translation)."""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Translate app_en.arb to another locale")
    parser.add_argument("target", help="Target locale code, e.g. de, it")
    args = parser.parse_args()

    out_path = ROOT / "lib" / "l10n" / f"app_{args.target}.arb"
    en: dict = json.loads(EN_PATH.read_text(encoding="utf-8"))
    out: dict = {"@@locale": args.target}
    ...
    print(f"Translating {len(strings)} strings to {args.target}...", flush=True)
    ...
    for start in range(0, len(protected_list), BATCH):
        ...
        try:
            ...
            print(f"  batch {start} failed ({exc}), one-by-one", flush=True)
            batch_out = []
            for item in batch:
                try:
    ...
    for key in keys:
        ...
        if isinstance(value, str):
            out[key] = lookup.get(value, value)
            meta_key = f"@{key}"
            if meta_key in en:
                out[meta_key] = en[meta_key]
        else:
            out[key] = value
